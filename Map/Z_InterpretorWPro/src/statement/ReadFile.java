...
public class ReadFile implements IStatement 
{
	...
	public PrgState execute(PrgState ps) 
	{
		try 
		{
			int id = exprFileId.eval(ps.getStackOfSym().first(),ps.getHeap());
			if(!exists(ps,id))
			...
			if(ps.getStackOfSym().first().contains(varName))
				ps.getStackOfSym().first().update(varName,result);
			else
				ps.getStackOfSym().first().add(varName,result);
		} 
		catch (IOException e) 
		{
