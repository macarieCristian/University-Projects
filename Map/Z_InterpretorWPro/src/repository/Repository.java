	...
	public void logPrgStateExec(PrgState ps) 
	{
		try(PrintWriter pw = new PrintWriter(new BufferedWriter(new FileWriter(logFile,true))))
		{
			...
			for(IStatement st : currentState.getExeStack().getAll())
				pw.println(st);
			pw.println("SymTable:");
			for(String key : currentState.getStackOfSym().first().getAll())
				pw.println(key+" -> "+currentState.getStackOfSym().first().getValue(key));
			pw.println("Out:");
			...
			pw.println("LockTable:");
			for(String key : currentState.getProc().getAll())
				pw.println(key+" -> "+currentState.getProc().getValue(key));
			pw.println("---------------------------------------------------------------");
		} 
		catch (IOException e) 
		{
