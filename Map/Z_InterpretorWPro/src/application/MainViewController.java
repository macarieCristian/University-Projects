...
public class MainViewController 
{
	...
	private ObservableList<IStatement> getList()
	{
		...
		IStatement stm21 = new CompStatement(new AssignStatement("v",new ConstExpression(2)),
				new CompStatement(new AssignStatement("w",new ConstExpression(5)),
				new CompStatement(new Call("sum",new ArithmeticExpression('*',new VarExpression("v"),new ConstExpression(10)),new VarExpression("w")),
				new CompStatement(new PrintStatement(new VarExpression("v")),
				new CompStatement(new ForkStatement(new Call("product",new VarExpression("v"),new VarExpression("w"))),
						new ForkStatement(new Call("sum",new VarExpression("v"),new VarExpression("w"))))))));
				
						
		
		ObservableList<IStatement> list = FXCollections.observableArrayList();
		list.addAll(stm,stm1,stm2,stm3,stm4,stm5,stm6,stm7,stm8,stmex,stm9,stm10,stm11,stm12,stm13,stm14,stm16,stm18,stm19,stm20,stm21);
		return list;
	}
